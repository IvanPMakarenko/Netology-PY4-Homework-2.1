with open('list_of_dishes.txt') as f:
	# print(f.read())
	cook_book = {}
	for line in f:
		...
		i = 0
		ingridients_list = []
		while i < count_ingridients:
			i += 1
			ingridient = f.readline().lower().split('|')
			ingridients = {'ingridient_name': None, 'quantity': None, 'measure': None}
...
def check_dish_in_cook_book(dishes):
	...
	return real_dishes #добавил новую функцию
...
def create_shop_list():
  ...
  if len(real_dishes) == 0:
  	...
  	print('В меню только:', *real_dishes)
  else:
